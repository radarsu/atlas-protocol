...
- Tiered RandomX-based Proof-of-Work
...
Atlas-Proofs: [{"data":{"@type":"Intangible","name":"atlas:ProofOfWork","identifier":{"@type":"PropertyValue","propertyID":"pow","value":"<32-hex nonce>"}},"signature":"<Base64Signature>"}]
Atlas-Signature: t=<Method|UnixTimestampMs|PathWithQuery>; s=<Base64Signature>
```

`Atlas-Proofs` is a JSON array of signed proof envelopes. `atlas:ProofOfWork` is mandatory at the protocol layer. Additional proof envelopes, such as delegated-key permit proofs, MAY be included alongside it.
...
4. For each supported tier (in deterministic order):
    ...
    - Derive `timeCode`
    - Rebuild `challenge`
    - Recompute Argon2 using `Atlas-Proofs[*].data.identifier.value` from the `atlas:ProofOfWork` proof envelope
