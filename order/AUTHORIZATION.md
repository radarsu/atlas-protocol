...
- **Header**: `Atlas-Challenge: <base64(random bytes)>`
- **Header**: `Atlas-Expires: <unit timestamp milliseconds>`
...
Response:
  401 Unauthorized
  Atlas-Challenge: base64(random bytes)
  Atlas-Expires: 1770065816302
```

On the next request, a rejected client (if the user is trusted by any of the listed authorities) SHOULD include:

- **Header**: `Atlas-Public-Key: <Public key>`
- **Header**: `Atlas-Signature: <Signature over challenge verifiable against Atlas-Public-Key>`
...
Request:
  ...
  Atlas-Signature: <Signature over challenge>
```

Then the Node verifies:

- **Challenge validity**: if `Atlas-Expires` has not expired.
- **Signature validity**: `Atlas-Signature` is a valid signature by `Atlas-Public-Key` over the challenged bytes (`Atlas-Challenge` value).

If verification fails, the Node rejects the request with `401 Unauthorized` and includes a new `Atlas-Challenge`.

If verification passes, the Node accepts the request.
